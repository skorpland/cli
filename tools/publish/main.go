...
func publishPackages(ctx context.Context, version string, beta bool) error {
	...
	config.FormulaName = "Powerbase"
	config.Description = "Powerbase CLI"
	filename := "powerbase"
	if beta {
...
func fetchConfig(ctx context.Context, version string) (PackageConfig, error) {
	...
	checkPath := fmt.Sprintf("/%s/%s/releases/download/v%[3]s/powerbase_%[3]s_checksums.txt",
		utils.CLI_OWNER,
		utils.CLI_REPO,
		version,
	)
...
func updatePackage(ctx context.Context, client *github.Client, repo, path string, tmpl *template.Template, config PackageConfig) error {
	...
	branch := "release/cli"
	master := "main"
	if err := shared.CreateGitBranch(ctx, client, POWERBASE_OWNER, repo, branch, master); err != nil {
		return err
	}
	// Get file SHA
	opts := github.RepositoryContentGetOptions{Ref: "heads/" + branch}
	file, _, _, err := client.Repositories.GetContents(ctx, POWERBASE_OWNER, repo, path, &opts)
	if err != nil {
		return err
	}
	...
	resp, _, err := client.Repositories.UpdateFile(ctx, POWERBASE_OWNER, repo, path, &commit)
	if err != nil {
		return err
	}
	...
	return shared.CreatePullRequest(ctx, client, POWERBASE_OWNER, repo, pr)
}
