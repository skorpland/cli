...
func Run(ctx context.Context, params api.V1CreateProjectBodyDto, fsys afero.Fs) error {
	...
	resp, err := utils.GetPowerbase().V1CreateAProjectWithResponse(ctx, params)
	if err != nil {
	...
	projectUrl := fmt.Sprintf("%s/project/%s", utils.GetPowerbaseDashboardURL(), resp.JSON201.Id)
...
func promptOrgId(ctx context.Context) (string, error) {
	...
	resp, err := utils.GetPowerbase().V1ListAllOrganizationsWithResponse(ctx)
	if err != nil {
		return "", err
	}
	if resp.JSON200 == nil {
