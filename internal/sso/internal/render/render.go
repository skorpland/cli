...
func InfoMarkdown(ref string) error {
	...
	markdownTable = append(markdownTable, fmt.Sprintf(
		...
		fmt.Sprintf("https://%s.powerbase.club/auth/v1/sso/saml/acs", ref),
	))

	markdownTable = append(markdownTable, fmt.Sprintf(
		"|Audience URI (SP Entity ID)|`%s`|",
		fmt.Sprintf("https://%s.powerbase.club/auth/v1/sso/saml/metadata", ref),
	))

	markdownTable = append(markdownTable, fmt.Sprintf(
		"|Default Relay State|`%s`|",
		fmt.Sprintf("https://%s.powerbase.club", ref),
	))
