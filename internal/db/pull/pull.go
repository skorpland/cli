...
var (
	...
	suggestExtraPull = fmt.Sprintf(
		...
		utils.Bold("auth"),
		utils.Bold("storage"),
		utils.Aqua("powerbase db pull --schema auth,storage"),
	)
)
...
func suggestMigrationRepair(extraRemote, extraLocal []string) string {
	...
	for _, version := range extraRemote {
		result += fmt.Sprintln(utils.Bold("powerbase migration repair --status reverted " + version))
	}
	for _, version := range extraLocal {
		result += fmt.Sprintln(utils.Bold("powerbase migration repair --status applied " + version))
	}
	return result
}
