...
var (
	...
	//go:embed templates/_powerbase.sql
	_powerbaseSchema string
...
cat <<'EOF' > /docker-entrypoint-initdb.d/powerbase_schema.sql && \
cat <<'EOF' >> /etc/postgresql/postgresql.conf && \
docker-entrypoint.sh postgres -D /etc/postgresql
` + _powerbaseSchema + `
EOF
...
EOF`}
	...
	if utils.NoBackupVolume {
		...
		utils.CmdSuggestion = fmt.Sprintf("Run %s to remove existing docker volumes.", utils.Aqua("powerbase stop --no-backup"))
...
func initStorageJob(host string) utils.DockerJob {
	return utils.DockerJob{
		...
		Env: []string{
			...
			fmt.Sprintf("DATABASE_URL=postgresql://powerbase_storage_admin:%s@%s:5432/postgres", utils.Config.Db.Password, host),
...
func initAuthJob(host string) utils.DockerJob {
	return utils.DockerJob{
		...
		Env: []string{
			...
			fmt.Sprintf("GOTRUE_DB_DATABASE_URL=postgresql://powerbase_auth_admin:%s@%s:5432/postgres", utils.Config.Db.Password, host),
