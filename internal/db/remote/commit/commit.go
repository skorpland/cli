...
func Run(ctx context.Context, schema []string, config pgconn.Config, fsys afero.Fs) error {
	...
	fmt.Println("Finished " + utils.Aqua("powerbase db remote commit") + ".")
	return nil
}
...
- Pushing unapplied migrations with ` + utils.Aqua("powerbase db push") + `,
- Or failing that, manually editing powerbase_migrations.schema_migrations table with ` + utils.Aqua("powerbase migration repair") + ".")
