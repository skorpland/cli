...
import (
	...
	"github.com/skorpland/cli/internal/gen/keys"
	"github.com/skorpland/cli/internal/migration/apply"
	"github.com/skorpland/cli/internal/migration/repair"
	...
	"github.com/skorpland/cli/pkg/vault"
)
...
func Run(ctx context.Context, version string, config pgconn.Config, fsys afero.Fs, options ...func(*pgx.ConnConfig)) error {
	...
	// Seed objects from powerbase/buckets directory
	...
	fmt.Fprintln(os.Stderr, "Finished "+utils.Aqua("powerbase db reset")+" on branch "+utils.Aqua(branch)+".")
	return nil
}
...
func initDatabase(ctx context.Context, options ...func(*pgx.ConnConfig)) error {
	conn, err := utils.ConnectLocalPostgres(ctx, pgconn.Config{User: "powerbase_admin"}, options...)
	if err != nil {
		return err
	}
	defer conn.Close(context.Background())
...
func recreateDatabase(ctx context.Context, options ...func(*pgx.ConnConfig)) error {
	conn, err := utils.ConnectLocalPostgres(ctx, pgconn.Config{User: "powerbase_admin", Database: "template1"}, options...)
	if err != nil {
		return err
	}
	defer conn.Close(context.Background())
	...
	sql := migration.MigrationFile{
		Statements: []string{
			...
			"DROP DATABASE IF EXISTS _powerbase WITH (FORCE)",
...
const (
	TERMINATE_BACKENDS      = "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname IN ('postgres', '_powerbase')"
	COUNT_REPLICATION_SLOTS = "SELECT COUNT(*) FROM pg_replication_slots WHERE database IN ('postgres', '_powerbase')"
)
...
func DisconnectClients(ctx context.Context, conn *pgx.Conn) error {
	...
	disconn := migration.MigrationFile{
		Statements: []string{
			...
			"ALTER DATABASE _powerbase ALLOW_CONNECTIONS false",
			TERMINATE_BACKENDS,
		},
	}
