...
import (
	...
	"github.com/skorpland/cli/internal/postgresConfig/get"
...
func Run(ctx context.Context, projectRef string, configKeys []string, noRestart bool, fsys afero.Fs) error {
	...
	resp, err := utils.GetPowerbase().V1UpdatePostgresConfigWithBodyWithResponse(ctx, projectRef, "application/json", bytes.NewReader(bts))
	if err != nil {
		return errors.Errorf("failed to update config overrides: %w", err)
	}
	if resp.JSON200 == nil {
		if resp.StatusCode() == 400 {
			return errors.Errorf("failed to update config overrides: %s (%s). This usually indicates that an unsupported or invalid config override was attempted. Please refer to https://powerbase.club/docs/guides/platform/custom-postgres-config", resp.Status(), string(resp.Body))
		}
