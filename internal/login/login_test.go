...
func TestLoginCommand(t *testing.T) {
	...
	t.Run("goes through automated flow successfully", func(t *testing.T) {
		...
		gock.New(utils.GetPowerbaseAPIHost()).
		...
		expectedBrowserUrl := fmt.Sprintf("%s/cli/login?session_id=%s&token_name=%s&public_key=%s", utils.GetPowerbaseDashboardURL(), sessionId, tokenName, publicKey)
