...
import (
	...
	cliConfig "github.com/skorpland/cli/pkg/config"
...
func Run(ctx context.Context, projectRef string, fsys afero.Fs, options ...func(*pgx.ConnConfig)) error {
	...
	fmt.Fprintln(os.Stdout, "Finished "+utils.Aqua("powerbase link")+".")
...
func linkPostgrest(ctx context.Context, projectRef string) error {
	resp, err := utils.GetPowerbase().V1GetPostgrestServiceConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkGotrue(ctx context.Context, projectRef string) error {
	resp, err := utils.GetPowerbase().V1GetAuthServiceConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkStorage(ctx context.Context, projectRef string) error {
	resp, err := utils.GetPowerbase().V1GetStorageConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkDatabaseSettings(ctx context.Context, projectRef string) error {
	resp, err := utils.GetPowerbase().V1GetPostgresConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkPooler(ctx context.Context, projectRef string, fsys afero.Fs) error {
	resp, err := utils.GetPowerbase().V1GetPowerpoolerConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func updatePoolerConfig(config api.PowerpoolerConfigResponse) {
...
func checkRemoteProjectStatus(ctx context.Context, projectRef string, fsys afero.Fs) error {
	resp, err := utils.GetPowerbase().V1GetProjectWithResponse(ctx, projectRef)
	if err != nil {
	...
	case api.V1ProjectWithDatabaseResponseStatusINACTIVE:
		utils.CmdSuggestion = fmt.Sprintf("An admin must unpause it from the Powerbase dashboard at %s", utils.Aqua(fmt.Sprintf("%s/project/%s", utils.GetPowerbaseDashboardURL(), projectRef)))
