...
func Run(ctx context.Context, projectRef string, args []string, fsys afero.Fs) error {
	if len(args) == 0 {
		...
		for _, secret := range secrets {
			if !strings.HasPrefix(secret.Name, "POWERBASE_") {
	...
	// 2. Unset secret(s).
	resp, err := utils.GetPowerbase().V1BulkDeleteSecretsWithResponse(ctx, projectRef, args)
	if err != nil {
	...
	fmt.Println("Finished " + utils.Aqua("powerbase secrets unset") + ".")
	return nil
}
