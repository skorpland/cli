...
func Run(ctx context.Context, projectRef, envFilePath string, args []string, fsys afero.Fs) error {
	...
	// 2. Set secret(s).
	resp, err := utils.GetPowerbase().V1BulkCreateSecretsWithResponse(ctx, projectRef, secrets)
	if err != nil {
	...
	fmt.Println("Finished " + utils.Aqua("powerbase secrets set") + ".")
	return nil
}
...
func ListSecrets(envFilePath string, fsys afero.Fs, envArgs ...string) ([]api.CreateSecretBody, error) {
	...
	for name, value := range envMap {
		...
		if strings.HasPrefix(name, "POWERBASE_") {
			fmt.Fprintln(os.Stderr, "Env name cannot start with POWERBASE_, skipping: "+name)
			continue
		}
