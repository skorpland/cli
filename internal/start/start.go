...
import (
	...
	"github.com/skorpland/cli/internal/db/start"
	"github.com/skorpland/cli/internal/functions/serve"
	"github.com/skorpland/cli/internal/seed/buckets"
...
func Run(ctx context.Context, fsys afero.Fs, excludedContainers []string, ignoreHealthCheck bool) error {
	// Sanity checks.
	{
		...
		if err := utils.AssertPowerbaseDbIsRunning(); err == nil {
			fmt.Fprintln(os.Stderr, utils.Aqua("powerbase start")+" is already running.")
...
func run(p utils.Program, ctx context.Context, fsys afero.Fs, excludedContainers []string, dbConfig pgconn.Config, options ...func(*pgx.ConnConfig)) error {
	...
	if utils.Config.Analytics.Enabled && !isContainerExcluded(utils.Config.Analytics.Image, excluded) {
		...
		case config.LogflarePostgres:
			env = append(env,
				fmt.Sprintf("POSTGRES_BACKEND_URL=postgresql://%s:%s@%s:%d/%s", dbConfig.User, dbConfig.Password, dbConfig.Host, dbConfig.Port, "_powerbase"),
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					"KONG_DNS_ORDER=LAST,A,CNAME", // https://github.com/skorpland/cli/issues/14
...
`},
	...
	if utils.Config.Auth.Enabled && !isContainerExcluded(utils.Config.Auth.Image, excluded) {
		...
		env := []string{
			...
			fmt.Sprintf("GOTRUE_DB_DATABASE_URL=postgresql://powerbase_auth_admin:%s@%s:%d/%s", dbConfig.Password, dbConfig.Host, dbConfig.Port, dbConfig.Database),
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					fmt.Sprintf("DATABASE_URL=postgresql://powerbase_storage_admin:%s@%s:%d/%s", dbConfig.Password, dbConfig.Host, dbConfig.Port, dbConfig.Database),
					fmt.Sprintf("FILE_SIZE_LIMIT=%v", utils.Config.Storage.FileSizeLimit),
					"STORAGE_BACKEND=file",
					"FILE_STORAGE_BACKEND_PATH=" + dockerStoragePath,
					"TENANT_ID=stub",
					// TODO: https://github.com/skorpland/storage-api/issues/55
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					"POWERBASE_URL=http://" + utils.KongId + ":8000",
					"POWERBASE_PUBLIC_URL=" + utils.Config.Studio.ApiUrl,
					"AUTH_JWT_SECRET=" + utils.Config.Auth.JwtSecret.Value,
					"POWERBASE_ANON_KEY=" + utils.Config.Auth.AnonKey.Value,
					"POWERBASE_SERVICE_KEY=" + utils.Config.Auth.ServiceRoleKey.Value,
					"LOGFLARE_API_KEY=" + utils.Config.Analytics.ApiKey,
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					fmt.Sprintf("DATABASE_URL=ecto://%s:%s@%s:%d/%s", dbConfig.User, dbConfig.Password, dbConfig.Host, dbConfig.Port, "_powerbase"),
				...
				Cmd: []string{
					"/bin/sh", "-c",
					fmt.Sprintf("/app/bin/migrate && /app/bin/powerpooler eval '%s' && /app/bin/server", poolerTenantBuf.String()),
				},
				ExposedPorts: nat.PortSet{
					"4000/tcp": {},
