...
func getFunctionMetadata(ctx context.Context, projectRef, slug string) (*api.FunctionSlugResponse, error) {
	resp, err := utils.GetPowerbase().V1GetAFunctionWithResponse(ctx, projectRef, slug)
	if err != nil {
	...
	case http.StatusNotFound:
		return nil, errors.Errorf("Function %s does not exist on the Powerbase project.", utils.Aqua(slug))
	case http.StatusOK:
		break
	default:
		return nil, errors.Errorf("Failed to download Function %s on the Powerbase project: %s", utils.Aqua(slug), string(resp.Body))
	}
...
func downloadFunction(ctx context.Context, projectRef, slug, extractScriptPath string) error {
	...
	resp, err := utils.GetPowerbase().V1GetAFunctionBodyWithResponse(ctx, projectRef, slug)
	if err != nil {
...
func downloadOne(ctx context.Context, slug, projectRef string, fsys afero.Fs) (string, error) {
	fmt.Println("Downloading " + utils.Bold(slug))
	resp, err := utils.GetPowerbase().V1GetAFunctionBody(ctx, projectRef, slug)
	if err != nil {
		return "", errors.Errorf("failed to get function body: %w", err)
	}
...
func suggestLegacyBundle(slug string) string {
	return fmt.Sprintf("\nIf your function is deployed using CLI < 1.120.0, trying running %s instead.", utils.Aqua("powerbase functions download --legacy-bundle "+slug))
}
