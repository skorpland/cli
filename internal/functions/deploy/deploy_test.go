...
`)
		...
		importMapPath, err := filepath.Abs(filepath.Join(utils.PowerbaseDirPath, "import_map.json"))
		require.NoError(t, err)
...
`)
	...
	t.Run("throws error on empty functions", func(t *testing.T) {
		...
		err := Run(context.Background(), nil, true, nil, "", 1, fsys)
		// Check error
		assert.ErrorContains(t, err, "No Functions specified or found in powerbase/functions")
	})
