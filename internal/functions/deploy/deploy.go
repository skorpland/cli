...
func Run(ctx context.Context, slugs []string, useDocker bool, noVerifyJWT *bool, importMapPath string, maxJobs uint, fsys afero.Fs) error {
	...
	api := function.NewEdgeRuntimeAPI(flags.ProjectRef, *utils.GetPowerbase(), opt)
	...
	url := fmt.Sprintf("%s/project/%v/functions", utils.GetPowerbaseDashboardURL(), flags.ProjectRef)
...
func GetFunctionConfig(slugs []string, importMapPath string, noVerifyJWT *bool, fsys afero.Fs) (config.FunctionConfig, error) {
	...
	if len(functionsUsingDeprecatedGlobalFallback) > 0 {
		...
		fmt.Fprintln(os.Stderr,
			...
			utils.Aqua("https://powerbase.club/docs/guides/functions/import-maps"),
		)
	}
