...
const JWT_SECRET = Deno.env.get("POWERBASE_INTERNAL_JWT_SECRET")!;
const HOST_PORT = Deno.env.get("POWERBASE_INTERNAL_HOST_PORT")!;
const DEBUG = Deno.env.get("POWERBASE_INTERNAL_DEBUG") === "true";
const FUNCTIONS_CONFIG_STRING = Deno.env.get(
  "POWERBASE_INTERNAL_FUNCTIONS_CONFIG",
)!;

const WALLCLOCK_LIMIT_SEC = parseInt(
  Deno.env.get("POWERBASE_INTERNAL_WALLCLOCK_LIMIT_SEC"),
);
...
Deno.serve({
  handler: async (req: Request) => {
    ...
    const envVars = Object.entries(envVarsObj)
      .filter(([name, _]) =>
        !EXCLUDED_ENVS.includes(name) && !name.startsWith("POWERBASE_INTERNAL_")
      );
