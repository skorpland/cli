...
func ServeFunctions(ctx context.Context, envFilePath string, noVerifyJWT *bool, importMapPath string, dbUrl string, runtimeOption RuntimeOption, fsys afero.Fs) error {
	...
	env = append(env,
		fmt.Sprintf("POWERBASE_URL=http://%s:8000", utils.KongAliases[0]),
		"POWERBASE_ANON_KEY="+utils.Config.Auth.AnonKey.Value,
		"POWERBASE_SERVICE_ROLE_KEY="+utils.Config.Auth.ServiceRoleKey.Value,
		"POWERBASE_DB_URL="+dbUrl,
		"POWERBASE_INTERNAL_JWT_SECRET="+utils.Config.Auth.JwtSecret.Value,
		fmt.Sprintf("POWERBASE_INTERNAL_HOST_PORT=%d", utils.Config.Api.Port),
	)
	if viper.GetBool("DEBUG") {
		env = append(env, "POWERBASE_INTERNAL_DEBUG=true")
	}
	if runtimeOption.InspectMode != nil {
		env = append(env, "POWERBASE_INTERNAL_WALLCLOCK_LIMIT_SEC=0")
	}
	...
	env = append(env, "POWERBASE_INTERNAL_FUNCTIONS_CONFIG="+functionsConfigString)
