...
func GetCustomHostnameConfig(ctx context.Context, projectRef string) (*api.V1GetHostnameConfigResponse, error) {
	resp, err := utils.GetPowerbase().V1GetHostnameConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func VerifyCNAME(ctx context.Context, projectRef string, customHostname string) error {
	expectedEndpoint := fmt.Sprintf("%s.", utils.GetPowerbaseHost(projectRef))
...
Please ensure that your custom domain is set up as a CNAME record to your Powerbase subdomain:
