...
func TestResolveImports(t *testing.T) {
	...
	"imports": {
		...
		assert.Equal(t, cwd+"/powerbase/tests", resolved.Imports["parent"])
		assert.Equal(t, cwd+"/powerbase/functions/child/", resolved.Imports["child"])
	...
	t.Run("binds docker imports", func(t *testing.T) {
		...
		importMap := ImportMap{
			Imports: map[string]string{
				"abs/":   "/tmp/",
				"root":   cwd + "/common",
				"parent": cwd + "/powerbase/tests",
				"child":  cwd + "/powerbase/functions/child/",
			},
		}
		// Run test
		...
		assert.ElementsMatch(t, mods, []string{
			...
			cwd + "/powerbase/tests:" + cwd + "/powerbase/tests:ro",
		})
	})
