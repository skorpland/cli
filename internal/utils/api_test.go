...
func TestLookupIP(t *testing.T) {
	...
	t.Run("resolves IPv6 recursively", func(t *testing.T) {
		...
		gock.New("https://1.1.1.1").
			Get("/dns-query").
			MatchParam("name", "api.powerbase.club").
			MatchHeader("accept", "application/dns-json").
			Reply(http.StatusOK).
			JSON(&cloudflare.DNSResponse{Answer: []cloudflare.DNSAnswer{
				{Type: cloudflare.TypeCNAME, Data: "powerbase-api.fly.dev."},
				{Type: cloudflare.TypeAAAA, Data: "2606:2800:220:1:248:1893:25c8:1946"},
			}})
		// Run test
		ip, err := FallbackLookupIP(context.Background(), "api.powerbase.club")
		// Validate output
		assert.NoError(t, err)
	...
	t.Run("empty on no answer", func(t *testing.T) {
		...
		ip, err := FallbackLookupIP(context.Background(), host)
		// Validate output
		assert.ErrorContains(t, err, "failed to locate valid IP for api.powerbase.club; resolves to []cloudflare.DNSAnswer(nil)")
		assert.Empty(t, ip)
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
}
...
func TestResolveCNAME(t *testing.T) {
	t.Run("resolves CNAMEs with CloudFlare", func(t *testing.T) {
		...
		gock.New("https://1.1.1.1").
			...
			Reply(http.StatusOK).
			JSON(&cloudflare.DNSResponse{Answer: []cloudflare.DNSAnswer{
				{Type: cloudflare.TypeCNAME, Data: "foobarbaz.powerbase.club"},
			}})
		// Run test
		cname, err := ResolveCNAME(context.Background(), host)
		// Validate output
		assert.Equal(t, "foobarbaz.powerbase.club", cname)
		assert.Nil(t, err)
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
	...
	t.Run("missing CNAMEs return an error", func(t *testing.T) {
		...
		assert.ErrorContains(t, err, "failed to locate appropriate CNAME record for api.powerbase.club")
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
