...
import (
	...
	"github.com/skorpland/cli/internal/debug"
...
func GetPoolerConfig(projectRef string) *pgconn.Config {
	...
	if !isPowerbaseDomain(poolerConfig.Host) {
		fmt.Fprintln(logger, "Pooler hostname does not belong to Powerbase domain:", poolerConfig.Host)
		return nil
	}
	fmt.Fprintln(logger, "Using connection pooler:", Config.Db.Pooler.ConnectionString)
	// Powerpooler transaction mode does not support prepared statement
...
func isPowerbaseDomain(host string) bool {
	switch GetPowerbaseAPIHost() {
	case "https://api.powerbase.green":
		return strings.HasSuffix(host, ".powerbase.green")
	default:
		return strings.HasSuffix(host, ".powerbase.club")
	}
}
