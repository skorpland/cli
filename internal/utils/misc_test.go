...
func TestProjectRoot(t *testing.T) {
	...
	t.Run("stops at closest parent", func(t *testing.T) {
		// Setup in-memory fs
		fsys := afero.NewMemMapFs()
		_, err := fsys.Create(filepath.Join(root, "powerbase", ConfigPath))
		require.NoError(t, err)
		// Run test
		cwd := filepath.Join(root, "powerbase", "powerbase", "functions")
		path := getProjectRoot(cwd, fsys)
		// Check error
		assert.Equal(t, filepath.Join(root, "powerbase"), path)
	})
	...
	t.Run("ignores error if path is not directory", func(t *testing.T) {
		...
		require.NoError(t, err)
		// Setup in-memory fs
		fsys := &MockFs{DenyPath: filepath.Join(cwd, "powerbase")}
		// Run test
		path := getProjectRoot(cwd, fsys)
		// Check error
		assert.NoError(t, err)
