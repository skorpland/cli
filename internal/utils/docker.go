...
const (
	...
	CliProjectLabel     = "com.powerbase.cli.project"
...
// Defaults to Powerbase public ECR for faster image pull
...
func GetRegistryImageUrl(imageName string) string {
	...
	return registry + "/powerbase/" + imageName
}
			...
			for _, p := range c.Ports {
				if fmt.Sprintf("%s:%d", p.IP, p.PublicPort) == hostPort {
					if project, ok := c.Labels[CliProjectLabel]; ok {
						return "\nTry stopping the running project with " + Aqua("powerbase stop --project-id "+project)
					} else {
						name := c.ID
						if len(c.Names) > 0 {
							name = c.Names[0]
