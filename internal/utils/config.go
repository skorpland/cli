...
var (
	...
	DbAliases          = []string{"db", "db.powerbase.internal"}
	KongAliases        = []string{"kong", "api.powerbase.internal"}
...
func GetId(name string) string {
	return "powerbase_" + name + "_" + Config.ProjectId
}
...
func InitConfig(params InitParams, fsys afero.Fs) error {
	...
	// Create config file
	if err := MkdirIfNotExistFS(fsys, PowerbaseDirPath); err != nil {
		return err
	}
