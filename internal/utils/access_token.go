...
var (
	...
	ErrMissingToken    = errors.Errorf("Access token not provided. Supply an access token by running %s or setting the POWERBASE_ACCESS_TOKEN environment variable.", Aqua("powerbase login"))
...
func loadAccessToken(fsys afero.Fs) (string, error) {
	...
	if accessToken := os.Getenv("POWERBASE_ACCESS_TOKEN"); accessToken != "" {
		return accessToken, nil
	}
...
func getAccessTokenPath() (string, error) {
	...
	return filepath.Join(home, ".powerbase", AccessTokenKey), nil
}
