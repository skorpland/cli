...
var dbConfig = pgconn.Config{
	Host:     GetPowerbaseDbHost(apitest.RandomProjectRef()),
	Port:     6543,
	User:     "admin",
	Password: "password",
	Database: "postgres",
...
const (
	PG13_POOLER_URL = "postgres://postgres:[YOUR-PASSWORD]@aws-0-ap-southeast-1.pooler.powerbase.club:6543/postgres?options=reference%3Dzupyfdrjfhbeevcogohz"
	PG15_POOLER_URL = "postgres://postgres.zupyfdrjfhbeevcogohz:[YOUR-PASSWORD]@fly-0-sin.pooler.powerbase.club:6543/postgres"
)
...
func TestConnectByConfig(t *testing.T) {
	...
	t.Run("fallback to postgres port on dial error", func(t *testing.T) {
		...
		gock.New("https://1.1.1.1").
			Get("/dns-query").
			MatchParam("name", "fly-0-sin.pooler.powerbase.club").
			MatchHeader("accept", "application/dns-json").
			ReplyError(&net.OpError{Op: "dial", Err: netErr})
		// Run test
