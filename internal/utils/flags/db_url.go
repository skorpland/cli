...
func ParseDatabaseConfig(flagSet *pflag.FlagSet, fsys afero.Fs) error {
	...
	case proxy:
		...
		DbConfig.Host = utils.GetPowerbaseAPIHost()
		DbConfig.Port = 443
		DbConfig.User = "postgres"
...
func getPassword(projectRef string) string {
	...
	resetUrl := fmt.Sprintf("%s/project/%s/settings/database", utils.GetPowerbaseDashboardURL(), projectRef)
...
func getDbConfig(projectRef string) pgconn.Config {
	...
	return pgconn.Config{
		Host:     utils.GetPowerbaseDbHost(projectRef),
		Port:     5432,
		User:     "postgres",
		Database: "postgres",
	}
