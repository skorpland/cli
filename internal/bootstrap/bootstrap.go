...
import (
	...
	"github.com/skorpland/cli/internal/db/push"
	initBlank "github.com/skorpland/cli/internal/init"
	"github.com/skorpland/cli/internal/link"
	"github.com/skorpland/cli/internal/login"
	"github.com/skorpland/cli/internal/projects/apiKeys"
	"github.com/skorpland/cli/internal/projects/create"
...
func checkProjectHealth(ctx context.Context) error {
	...
	resp, err := utils.GetPowerbase().V1GetServicesHealthWithResponse(ctx, flags.ProjectRef, &params)
	if err != nil {
		return err
	}
	if resp.JSON200 == nil {
...
const (
	POWERBASE_SERVICE_ROLE_KEY = "POWERBASE_SERVICE_ROLE_KEY"
	POWERBASE_ANON_KEY         = "POWERBASE_ANON_KEY"
	POWERBASE_URL              = "POWERBASE_URL"
	...
	NEXT_PUBLIC_POWERBASE_ANON_KEY = "NEXT_PUBLIC_POWERBASE_ANON_KEY"
	NEXT_PUBLIC_POWERBASE_URL      = "NEXT_PUBLIC_POWERBASE_URL"
	EXPO_PUBLIC_POWERBASE_ANON_KEY = "EXPO_PUBLIC_POWERBASE_ANON_KEY"
	EXPO_PUBLIC_POWERBASE_URL      = "EXPO_PUBLIC_POWERBASE_URL"
)
...
func writeDotEnv(keys []api.ApiKeyResponse, config pgconn.Config, fsys afero.Fs) error {
	...
	initial := map[string]string{
		POWERBASE_URL: "https://" + utils.GetPowerbaseHost(flags.ProjectRef),
	...
	for k, v := range envs {
		switch k {
		case POWERBASE_SERVICE_ROLE_KEY:
		case POWERBASE_ANON_KEY:
		case POWERBASE_URL:
		case POSTGRES_URL:
		// Derived keys
		...
		case NEXT_PUBLIC_POWERBASE_ANON_KEY:
			fallthrough
		case EXPO_PUBLIC_POWERBASE_ANON_KEY:
			initial[k] = initial[POWERBASE_ANON_KEY]
		case NEXT_PUBLIC_POWERBASE_URL:
			fallthrough
		case EXPO_PUBLIC_POWERBASE_URL:
			initial[k] = initial[POWERBASE_URL]
		default:
			initial[k] = v
		}
	}
...
func ListSamples(ctx context.Context, client *github.Client) ([]StarterTemplate, error) {
	owner := "powerbase-community"
	repo := "powerbase-samples"
	path := "samples.json"
	ref := "main"
...
func downloadSample(ctx context.Context, client *github.Client, templateUrl string, fsys afero.Fs) error {
	...
	// https://github.com/skorpland/powerbase/tree/master/examples/user-management/nextjs-user-management
