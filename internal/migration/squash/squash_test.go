...
import (
	...
	"github.com/skorpland/cli/internal/testing/apitest"
	"github.com/skorpland/cli/internal/testing/fstest"
	"github.com/skorpland/cli/internal/testing/helper"
...
func TestSquashCommand(t *testing.T) {
	...
	t.Run("baselines migration history", func(t *testing.T) {
		...
		defer conn.Close(t)
		helper.MockMigrationHistory(conn).
			Query(fmt.Sprintf("DELETE FROM powerbase_migrations.schema_migrations WHERE version <=  '0' ;INSERT INTO powerbase_migrations.schema_migrations(version, name, statements) VALUES( '0' ,  'init' ,  '{%s}' )", sql)).
			Reply("INSERT 0 1")
		// Run test
...
func TestBaselineMigration(t *testing.T) {
	...
	t.Run("throws error on query failure", func(t *testing.T) {
		...
		defer conn.Close(t)
		helper.MockMigrationHistory(conn).
			Query(fmt.Sprintf("DELETE FROM powerbase_migrations.schema_migrations WHERE version <=  '%[1]s' ;INSERT INTO powerbase_migrations.schema_migrations(version, name, statements) VALUES( '%[1]s' ,  'init' ,  null )", "0")).
			ReplyError(pgerrcode.InsufficientPrivilege, "permission denied for relation powerbase_migrations")
		// Run test
		err := baselineMigrations(context.Background(), dbConfig, "0", fsys, conn.Intercept, func(cc *pgx.ConnConfig) {
			cc.PreferSimpleProtocol = true
		})
		// Check error
		assert.ErrorContains(t, err, `ERROR: permission denied for relation powerbase_migrations (SQLSTATE 42501)`)
	})
