...
func TestPendingMigrations(t *testing.T) {
	...
	t.Run("throws error on missing local migration", func(t *testing.T) {
		...
		assert.Contains(t, utils.CmdSuggestion, "powerbase migration repair --status reverted 0")
	})
...
func TestIgnoreVersionMismatch(t *testing.T) {
	...
	t.Run("throws error on missing local and remote migration", func(t *testing.T) {
		...
		// Check error
		assert.ErrorIs(t, err, migration.ErrMissingLocal)
		assert.Contains(t, utils.CmdSuggestion, "powerbase migration repair --status reverted 20221201000004")
	})
	...
	t.Run("throws error on missing local migration", func(t *testing.T) {
		...
		// Check error
		assert.ErrorIs(t, err, migration.ErrMissingLocal)
		assert.Contains(t, utils.CmdSuggestion, "powerbase migration repair --status reverted 20221201000001 20221201000003 20221201000004")
	})
}
