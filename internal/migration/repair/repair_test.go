...
func TestRepairCommand(t *testing.T) {
	...
	t.Run("throws error on insert failure", func(t *testing.T) {
		...
		helper.MockMigrationHistory(conn).
			...
			ReplyError(pgerrcode.DuplicateObject, `relation "powerbase_migrations.schema_migrations" does not exist`)
		// Run test
		err := Run(context.Background(), dbConfig, []string{"0"}, Applied, fsys, conn.Intercept)
		// Check error
		assert.ErrorContains(t, err, `ERROR: relation "powerbase_migrations.schema_migrations" does not exist (SQLSTATE 42710)`)
	})
}
...
func TestRepairAll(t *testing.T) {
	t.Run("repairs whole history", func(t *testing.T) {
		...
		defer conn.Close(t)
		helper.MockMigrationHistory(conn).
			Query(migration.TRUNCATE_VERSION_TABLE + `;INSERT INTO powerbase_migrations.schema_migrations(version, name, statements) VALUES( '0' ,  'test' ,  '{select 1}' )`).
