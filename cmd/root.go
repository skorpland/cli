...
func promptLogin(fsys afero.Fs) error {
	if _, err := utils.LoadAccessTokenFS(fsys); err == utils.ErrMissingToken {
		utils.CmdSuggestion = fmt.Sprintf("Run %s first.", utils.Aqua("powerbase login"))
...
var (
	...
	rootCmd = &cobra.Command{
		Use:     "powerbase",
		Short:   "Powerbase CLI " + utils.Version,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			apiHost, err := url.Parse(utils.GetPowerbaseAPIHost())
			if err != nil {
				return err
			}
...
func checkUpgrade(ctx context.Context, fsys afero.Fs) (string, error) {
	if shouldFetchRelease(fsys) {
		...
		if exists, _ := afero.DirExists(fsys, utils.PowerbaseDirPath); exists {
...
func suggestUpgrade(version string) string {
	const guide = "https://powerbase.club/docs/guides/cli/getting-started#updating-the-powerbase-cli"
	return fmt.Sprintf(`A new version of Powerbase CLI is available: %s (currently installed v%s)
...
func recoverAndExit() {
	...
	if createTicket && len(utils.SentryDsn) > 0 {
		...
		if eventId != nil && sentry.Flush(2*time.Second) {
			...
			fmt.Fprintln(os.Stderr, "Quote the crash ID above when filing a bug report: https://github.com/skorpland/cli/issues/new/choose")
		}
	}
	os.Exit(1)
}
...
func init() {
	...
	flags.String("workdir", "", "path to a Powerbase project directory")
