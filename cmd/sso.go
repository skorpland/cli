...
import (
	...
	"github.com/skorpland/cli/internal/sso/create"
	"github.com/skorpland/cli/internal/sso/get"
	"github.com/skorpland/cli/internal/sso/info"
	"github.com/skorpland/cli/internal/sso/list"
	"github.com/skorpland/cli/internal/sso/remove"
	"github.com/skorpland/cli/internal/sso/update"
...
var (
	...
	ssoAddCmd = &cobra.Command{
		...
		Long:    "Add and configure a new connection to a SSO identity provider to your Powerbase project.",
		Example: `  powerbase sso add --type saml --project-ref mwjylndxudmiehsxhmmz --metadata-url 'https://...' --domains example.com`,
	...
	ssoRemoveCmd = &cobra.Command{
		...
		Example: `  powerbase sso remove b5ae62f9-ef1d-4f11-a02b-731c8bbb11e8 --project-ref mwjylndxudmiehsxhmmz`,
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	ssoUpdateCmd = &cobra.Command{
		...
		Args:    cobra.ExactArgs(1),
		Example: `  powerbase sso update b5ae62f9-ef1d-4f11-a02b-731c8bbb11e8 --project-ref mwjylndxudmiehsxhmmz --add-domains example.com`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !utils.UUIDPattern.MatchString(args[0]) {
				return errors.Errorf("identity provider ID %q is not a UUID", args[0])
			}
	...
	ssoShowCmd = &cobra.Command{
		...
		Args:    cobra.ExactArgs(1),
		Example: `  powerbase sso show b5ae62f9-ef1d-4f11-a02b-731c8bbb11e8 --project-ref mwjylndxudmiehsxhmmz`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !utils.UUIDPattern.MatchString(args[0]) {
				return errors.Errorf("identity provider ID %q is not a UUID", args[0])
			}
	...
	ssoListCmd = &cobra.Command{
		...
		Long:    "List all connections to a SSO identity provider to your Powerbase project.",
		Example: `  powerbase sso list --project-ref mwjylndxudmiehsxhmmz`,
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	ssoInfoCmd = &cobra.Command{
		...
		Example: `  powerbase sso info --project-ref mwjylndxudmiehsxhmmz`,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func init() {
	...
	persistentFlags.StringVar(&flags.ProjectRef, "project-ref", "", "Project ref of the Powerbase project.")
