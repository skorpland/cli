...
begin
  ...
  for rec in
    select *
    from pg_extension p
    where p.extname not in ('pg_graphql', 'pg_net', 'pg_stat_statements', 'pgcrypto', 'pgjwt', 'pgsodium', 'plpgsql', 'powerbase_vault', 'uuid-ossp')
  loop
  ...
  -- views (necessary for views referencing objects in Powerbase-managed schemas)
  for rec in
    select *
    from pg_class c
    where
  ...
  -- materialized views (necessary for materialized views referencing objects in Powerbase-managed schemas)
  for rec in
    select *
    from pg_class c
    where
  ...
  for rec in
    ...
    where
      ...
      or c.relnamespace::regnamespace::name = 'powerbase_functions' and c.relname != 'migrations'
      or c.relnamespace::regnamespace::name = 'powerbase_migrations')
      and c.relkind = 'r'
  loop
  ...
  for rec in
    ...
    where
      p.pubname not like 'powerbase_realtime%' and p.pubname not like 'realtime_messages%'
  loop
