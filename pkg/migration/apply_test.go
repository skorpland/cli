...
func TestApplyMigrations(t *testing.T) {
	...
	t.Run("throws error on missing table", func(t *testing.T) {
		...
		mockMigrationHistory(conn).
			Query(testSchema).
			ReplyError(pgerrcode.UndefinedTable, `relation "powerbase_migrations.schema_migrations" does not exist`).
			Query(INSERT_MIGRATION_VERSION, "0", "schema", []string{testSchema}).
			Reply("INSERT 0 1")
		// Run test
		err := ApplyMigrations(context.Background(), pending, conn.MockClient(t), testMigrations)
		// Check error
		assert.ErrorContains(t, err, `ERROR: relation "powerbase_migrations.schema_migrations" does not exist (SQLSTATE 42P01)`)
	})
}
