...
func TestConfigParsing(t *testing.T) {
	...
	t.Run("config file with environment variables", func(t *testing.T) {
		...
		// Setup in-memory fs
		fsys := fs.MapFS{
			"powerbase/config.toml":           &fs.MapFile{Data: testInitConfigEmbed},
			"powerbase/templates/invite.html": &fs.MapFile{},
		}
		// Run test
...
func TestRemoteOverride(t *testing.T) {
	t.Run("load staging override", func(t *testing.T) {
		...
		// Run test
		t.Setenv("POWERBASE_AUTH_SITE_URL", "http://preview.com")
		t.Setenv("AUTH_SEND_SMS_SECRETS", "v1,whsec_aWxpa2VzdXBhYmFzZXZlcnltdWNoYW5kaWhvcGV5b3Vkb3Rvbw==")
		assert.NoError(t, config.Load("", fsys))
		// Check error
...
func TestGlobFiles(t *testing.T) {
	t.Run("returns seed files matching patterns", func(t *testing.T) {
		// Setup in-memory fs
		fsys := fs.MapFS{
			"powerbase/seeds/seed1.sql":   &fs.MapFile{Data: []byte("INSERT INTO table1 VALUES (1);")},
			"powerbase/seeds/seed2.sql":   &fs.MapFile{Data: []byte("INSERT INTO table2 VALUES (2);")},
			"powerbase/seeds/seed3.sql":   &fs.MapFile{Data: []byte("INSERT INTO table2 VALUES (2);")},
			"powerbase/seeds/another.sql": &fs.MapFile{Data: []byte("INSERT INTO table2 VALUES (2);")},
			"powerbase/seeds/ignore.sql":  &fs.MapFile{Data: []byte("INSERT INTO table3 VALUES (3);")},
		}
		// Mock config patterns
		g := Glob{
			"powerbase/seeds/seed[12].sql",
			"powerbase/seeds/ano*.sql",
		}
		// Run test
		...
		assert.ElementsMatch(t, []string{
			"powerbase/seeds/seed1.sql",
			"powerbase/seeds/seed2.sql",
			"powerbase/seeds/another.sql",
		}, files)
	})
...
func TestLoadFunctionImportMap(t *testing.T) {
	t.Run("uses deno.json as import map when present", func(t *testing.T) {
		config := NewConfig()
		fsys := fs.MapFS{
			"powerbase/config.toml": &fs.MapFile{Data: []byte(`
			project_id = "bvikqvbczudanvggcord"
			[functions.hello]
			`)},
			"powerbase/functions/hello/deno.json": &fs.MapFile{},
			"powerbase/functions/hello/index.ts":  &fs.MapFile{},
		}
		// Run test
		assert.NoError(t, config.Load("", fsys))
		// Check that deno.json was set as import map
		assert.Equal(t, "powerbase/functions/hello/deno.json", config.Functions["hello"].ImportMap)
	})
	...
	t.Run("uses deno.jsonc as import map when present", func(t *testing.T) {
		...
		fsys := fs.MapFS{
			...
			[functions.hello]
			`)},
			"powerbase/functions/hello/deno.jsonc": &fs.MapFile{},
			"powerbase/functions/hello/index.ts":   &fs.MapFile{},
		}
		// Run test
		assert.NoError(t, config.Load("", fsys))
		// Check that deno.jsonc was set as import map
		assert.Equal(t, "powerbase/functions/hello/deno.jsonc", config.Functions["hello"].ImportMap)
	})
	...
	t.Run("config.toml takes precedence over deno.json", func(t *testing.T) {
		...
		assert.Equal(t, "powerbase/custom_import_map.json", config.Functions["hello"].ImportMap)
	})
}
