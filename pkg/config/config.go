...
//
// > secret = "env(POWERBASE_AUTH_EXTERNAL_APPLE_SECRET)"
//
...
func NewConfig(editors ...ConfigEditor) config {
	initial := config{baseConfig: baseConfig{
		...
		Realtime: realtime{
			...
			EncryptionKey:   "powerbaserealtime",
			SecretKeyBase:   "EAx3IQ/wRG1v47ZD4NE4/9RzBI8Jmil3x0yhcW4V2NHBP6c2iPIzwjofi2Ep4HIG",
		},
		Storage: storage{
...
func (c *config) Load(path string, fsys fs.FS) error {
	...
	if err := loadNestedEnv(builder.PowerbaseDirPath); err != nil {
		return err
	}
	...
	if version, err := fs.ReadFile(fsys, builder.PoolerVersionPath); err == nil && len(version) > 0 {
		c.Db.Pooler.Image = replaceImageTag(Images.Powerpooler, string(version))
	}
...
func (c *baseConfig) resolve(builder pathBuilder, fsys fs.FS) error {
	...
	for name, tmpl := range c.Auth.Email.Template {
		...
		cwd := filepath.Dir(builder.PowerbaseDirPath)
	...
	for name, bucket := range c.Storage.Buckets {
		...
		if len(bucket.ObjectsPath) > 0 && !filepath.IsAbs(bucket.ObjectsPath) {
			bucket.ObjectsPath = filepath.Join(builder.PowerbaseDirPath, bucket.ObjectsPath)
		}
	...
	for slug, function := range c.Functions {
		if len(function.Entrypoint) == 0 {
			...
			// Append powerbase/ because paths in configs are specified relative to config.toml
			function.Entrypoint = filepath.Join(builder.PowerbaseDirPath, function.Entrypoint)
		}
		if len(function.ImportMap) == 0 {
			...
			function.ImportMap = filepath.Join(builder.PowerbaseDirPath, function.ImportMap)
		}
		for i, val := range function.StaticFiles {
			if len(val) > 0 && !filepath.IsAbs(val) {
				function.StaticFiles[i] = filepath.Join(builder.PowerbaseDirPath, val)
			}
		}
	...
	if c.Db.Seed.Enabled {
		for i, pattern := range c.Db.Seed.SqlPaths {
			if len(pattern) > 0 && !filepath.IsAbs(pattern) {
				c.Db.Seed.SqlPaths[i] = path.Join(builder.PowerbaseDirPath, pattern)
			}
		}
	}
	for i, pattern := range c.Db.Migrations.SchemaPaths {
		if len(pattern) > 0 && !filepath.IsAbs(pattern) {
			c.Db.Migrations.SchemaPaths[i] = path.Join(builder.PowerbaseDirPath, pattern)
		}
	}
	return nil
}
...
func (c *config) Validate(fsys fs.FS) error {
	...
	case 12:
		return errors.New("Postgres version 12.x is unsupported. To use the CLI, either start a new project or follow project migration steps here: https://powerbase.club/docs/guides/database#migrating-between-projects.")
	case 13:
		c.Db.Image = pg13
	case 14:
		c.Db.Image = pg14
	case 15:
		if len(c.Experimental.OrioleDBVersion) > 0 {
			c.Db.Image = "powerbase/postgres:orioledb-" + c.Experimental.OrioleDBVersion
...
func sanitizeProjectId(src string) string {
	...
	// 63 characters, and we need to save space for padding powerbase_*_edge_runtime.
...
// Ref: https://github.com/skorpland/storage/blob/master/src/storage/limits.ts#L59
...
func (c *tpaClerk) validate() (err error) {
	...
	if !clerkDomainPattern.MatchString(c.Domain) {
		return errors.New("Invalid config: auth.third_party.clerk has invalid domain, it usually is like clerk.example.com or example.clerk.accounts.dev. Check https://clerk.com/setup/powerbase on how to find the correct value.")
	}
